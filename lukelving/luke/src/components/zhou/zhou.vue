  ...
  <div>
      <div class="banner">
        <van-swipe  :autoplay="3000" indicator-color="white">
          <van-swipe-item v-for="val in zhoubanner" :key="val.id"><img :src="val.img" alt=""/></van-swipe-item>
        </van-swipe>
      </div>
      <!-- 下拉选项 -->
      <van-dropdown-menu>
...
export default {
    ...
    mounted(){
        fetch("/static/zhoubanner.json").then((res)=>{
            return res.json();
        })
        .then(res=>{
            // console.log(res)
            this.zhoubanner=res.zhoubanner
        });
